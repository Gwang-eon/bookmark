...
- 기본값은 `local` 모드다.
- `local` 모드에서는 로컬 Chrome 경로 자동 탐지, 직접 적용, 백업, 롤백을 사용할 수 있다.
- `remote` 모드에서는 업로드 기반 분석/내보내기만 허용되고, 로컬 파일 경로 접근은 비활성화된다.

```bash
BOOKMARK_ACCESS_MODE=remote npm start
```
...
- 직접 적용은 현재 경로를 다시 분석한 결과에서만 허용된다.
